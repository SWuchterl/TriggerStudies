...
p.add_option('--input2',  type = 'string', default = "outBTag.FTKBtagging.ttbar.mwt2.All.root", dest = 'inFile2', help = 'intput File for V0' )
...
def doVarRatio(var, binning, xTitle, setLogy=1, minX=None, maxX=None, minY=None, etaRange=None):
    if etaRange:
        # offLF = getHist(inFile2,"offJets_matchedPuppi_L"+etaRange,          var,binning,ROOT.kBlack)
        offLF = getHist(inFile2,"offJets_matchedPuppiJet_L"+etaRange,          var,binning,ROOT.kBlack)
        ...
        # offBQ = getHist(inFile2,"offJets_matchedPuppi_B"+etaRange,          var,binning,ROOT.kRed+1)
        offBQ = getHist(inFile2,"offJets_matchedPuppiJet_B"+etaRange,          var,binning,ROOT.kRed+1)
        # hltBQ = getHist(inFile,"offJets_matchedPuppiJet_B"+etaRange,var,binning,ROOT.kRed+1)
        hltBQ = getHist(inFile,"offJets_matchedJet_B"+etaRange,var,binning,ROOT.kRed+1)
    else:
        # offLF = getHist(inFile2,"offJets_matchedPuppi_L",          var,binning,ROOT.kBlack)
        offLF = getHist(inFile2,"offJets_matchedPuppiJet_L",          var,binning,ROOT.kBlack)
        ...
        # offBQ = getHist(inFile2,"offJets_matchedPuppi_B",          var,binning,ROOT.kRed+1)
        offBQ = getHist(inFile2,"offJets_matchedPuppiJet_B",          var,binning,ROOT.kRed+1)
        # hltBQ = getHist(inFile,"offJets_matchedPuppiJet_B",var,binning,ROOT.kRed+1)
        hltBQ = getHist(inFile,"offJets_matchedJet_B",var,binning,ROOT.kRed+1)
    ...
    # leg.SetNColumns(2)
    leg.SetNColumns(1)
    leg.AddEntry(offBQ,"V0 tracks b-quark jets","L")
    leg.AddEntry(offLF,"V0 tracks light-flavor jets","L")
    ...
    cmsLines = getCMSText(xStart=0.2,yStart=0.85,subtext=o.cmsText, lumiText=o.lumiText)
    ...
    ratio_axis.GetYaxis().SetTitle("HLT to V0")
    ...
    LFRatio.GetYaxis().SetTitle("HLT to V0")
